...
return el[0]}},getParentID:(id)=>id.split("_").slice(0,-1).join("_"),getValue:(id)=>DOM.get(id).value,add:(html,el)=>{UTILS.contains(html,"<")?DOM.draw(el,html):DOM.render(html,el)},render:(html,el)=>{DOM.RENDER[html]=[el,html];DOM.update(html)},getRender:(name)=>DOM.RENDER[name],update:async function(el){var r=DOM.getRender(el)
...
target.id?this.elementChanged(target.id):value.id?this.elementChanged(value.id):target[0]&&target[0].id?this.elementChanged(target[0].id.split("_")[0]):0
...
return{add:(html,el)=>DOM.add(html,el),get:(el)=>DOM.get(el),h:(el)=>MOD.h.bind({}),HTML:()=>HTML,EVENTS:()=>EVENTS}}
