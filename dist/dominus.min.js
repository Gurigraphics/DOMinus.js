...
if(r){DOM.draw(r[0],MOD.h(HTML[r[1]]))}},draw:(el,content)=>{if(!content)DOM.none(el)
...
var DATA={}
var PROXY={keys:{},get(target,key){if(typeof target[key]==='object'&&target[key]!==null){return new Proxy(target[key],PROXY)}else return target[key]},set(target,key,value){var firstKey;this.keys={};target[key]=value
for(index in DATA)this.keys[index]=index
for(index in this.keys){for(index2 in DATA[index]){if(DATA[index][index2][key]){this.elementChanged(index)
return 0}}}
for(index in DATA)if(DATA[index][key])firstKey=index
if(firstKey)this.elementChanged(firstKey)
else this.elementChanged(key)
return!0},elementChanged(key){var currentPath=key
