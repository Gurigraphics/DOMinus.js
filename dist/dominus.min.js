...
if(data.html){if(HTML[data.html]){el+=">"+MOD.h(HTML[data.html])}
else el+=">"+data.html}else if(data.mount){if(HTML[data.mount]){if(UTILS.isArray(HTML[data.mount])){el+=">"+MOD.mountMap(HTML[data.mount])}else el+=">"+MOD.mount(HTML[data.mount])}else el+=">"+MOD.mount(data.mount[0])}else if(data.mountMap){if(HTML[data.mountMap])el+=">"+MOD.mountMap(HTML[data.mountMap])
else el+=">"+MOD.mountMap(data.mountMap[0])}else if(MOD.emptyElements.includes(data.tag)){return el+"/>"}
else{el+=">"}
...
var OBS={}
HTML=ObservableSlim.create(OBS,!0,function(changes){for(change in changes){var currentPath=changes[change].currentPath
